...
class TestCases(unittest.TestCase):
    ...
    db = DoMysql()
    #读取表格
    readexcel = ReadExcel(os.path.join(constant.data_dir, 'testcases.xlsx'), 'recharge')
    #获取表格内容
    ...
    def test_register(self,data_object):
        ...
        get_register = HttpRequest(method=data_object.method,url=url,data=eval(normal_data),cookies=get_data.cookies)
        #获取实际结果响应内容
        ...
        try:
            ...
            if start_res and after_res:
                ...
                self.assertEqual(eval(data_object.data)['amount'], float(change_money)) #断言实际余额与充值后数据库用户余额
