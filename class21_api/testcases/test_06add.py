...
class TestCases(unittest.TestCase):
    ...
    db=DoMysql()
    #读取数据表
    readexcel = ReadExcel(os.path.join(constant.data_dir, 'testcases.xlsx'), 'add')
    #获取数据
    ...
    def test_register(self,data_object):
        ...
        if data_object.check_sql: #若用例数据sql字段不为空则执行
            data_object.check_sql=self.get_data.get_replace(data_object.check_sql)
            db_res=self.db.find_count(data_object.check_sql) #获取受影响行数
        try:
            #断言实际结果与预期结果是否一致
            self.assertEqual(expected,res)
            if sql_res:  #若受影响行数不为空执行
                self.assertEqual(sql_res+1,db_res) #断言用例执行前和执行后受影响行数
