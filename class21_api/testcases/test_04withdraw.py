...
class TestCases(unittest.TestCase):
    ...
    db=DoMysql()
    #读取数据表
    readexcel = ReadExcel(os.path.join(constant.data_dir, 'testcases.xlsx'), 'withdraw')
    #获取测试数据
    ...
    def test_register(self,data_object):
        ...
        try:
            ...
            if start_res and after_res:
                change_money = start_res[0] - after_res[0]
                self.assertEqual(eval(data_object.data)['amount'], float(change_money)) #断言实际结果余额与数据库余额
